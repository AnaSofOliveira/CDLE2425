...
CORPUS_NAME=templateSegmentation
OUTPUT_FILE_NAME=showTemplates
...
if [[ $# -lt 1 ]]; then
	...
	case "$1" in
		...
		"HDFS")
			...
			INPUT=${BASE_DIRECTORY}/output/${CORPUS_NAME}
			OUTPUT=${BASE_DIRECTORY}/output/${OUTPUT_FILE_NAME}
			OUTPUT_IMAGES=${BASE_DIRECTORY}/output/${OUTPUT_FILE_NAME}_images

			INPUT_DIRECTORY=${FILE_SYSTEM_TYPE}${INPUT}
			OUTPUT_DIRECTORY=${FILE_SYSTEM_TYPE}${OUTPUT}
			OUTPUT_IMAGES_DIRECTORY=${FILE_SYSTEM_TYPE}${OUTPUT_IMAGES}

			LOCAL_OUTPUT_DIR="${HOME}/examples/output"
			echo -e "\nRemoving previous local output..."
			remove_output_local "${LOCAL_OUTPUT_DIR}/${OUTPUT_FILE_NAME}"

			LOCAL_OUTPUT_IMAGES_DIR="${HOME}/examples/output"
			echo -e "\nRemoving previous local output..."
			remove_output_local "${LOCAL_OUTPUT_DIR}/${OUTPUT_FILE_NAME}_images"
			...
			echo -e "\nRemoving previous output images..."
			remove_output "${OUTPUT_IMAGES}"
...
PYTHON_SRC="src/showTemplates"

JOB_FILE="${PYTHON_SRC}/ShowTemplates.py"
...
ARGS="${JOB_FILE} ${RUN_MODE}
	...
	--log-level ${LOG_LEVEL}
	--output-img ${OUTPUT_IMAGES_DIRECTORY}
	${INPUT_DIRECTORY} "
...
if [[ "$1" = "HDFS" ]]; then
	...
	echo -e "\nCopying output images files on HDFS file system to local output..."
	copy_file_local "${OUTPUT_IMAGES_DIRECTORY}" "${LOCAL_OUTPUT_IMAGES_DIR}"

	echo -e "Output images copied to local directory: ${LOCAL_OUTPUT_IMAGES_DIR}"
fi
