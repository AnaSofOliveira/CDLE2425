...
if [[ $# -lt 1 ]]; then
	...
	case "$1" in
		"local")
			...
			OUTPUT=${BASE_DIRECTORY}/output/templateSegmentation

			INPUT_DIRECTORY=${INPUT} # ${FILE_SYSTEM_TYPE}${INPUT}
			OUTPUT_DIRECTORY=${OUTPUT} # ${FILE_SYSTEM_TYPE}${OUTPUT}
		...
		"NFS")
			...
			INPUT_DIRECTORY=${INPUT_DIRECTORY_AUX} #${INPUT_FILE_SYSTEM_TYPE}${INPUT_DIRECTORY_AUX}
			
			OUTPUT_BASE_DIRECTORY=/user/${USER}
			OUTPUT=${OUTPUT_BASE_DIRECTORY}/output/templateSegmentation
...
JOB_FILE="templateSegmentation.py"
UTILS_FILE="utils/biospy_utils.py"

JOB_FILE_FULL="${PYTHON_SRC}/${JOB_FILE}"
UTILS_FILE_FULL="${PYTHON_SRC}/${UTILS_FILE}"

ARGS="${JOB_FILE_FULL} ${RUN_MODE} --files ${UTILS_FILE_FULL} --output-dir ${OUTPUT_DIRECTORY} ${INPUT_DIRECTORY}"
